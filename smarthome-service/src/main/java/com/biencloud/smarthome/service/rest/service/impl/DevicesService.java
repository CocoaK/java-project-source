...
public class DevicesService extends BaseResService<Device> implements
	...
	@Override
	public ResultEntity<String> updateByDeviceNo(Device device) {
		ResultEntity<String> re = new ResultEntity<String>(ResultEntity.FAILD,"","");
		if(device==null || StringUtils.isBlank(device.getDeviceCode())){
			re.setMessage("deviceNo is blank");
			return re;
		}
		Long deviceId = getIdByDeviceNo(device.getDeviceCode()).getData();
		if(deviceId==null){
			re.setContent(ResultEntity.NOT_EXIST, "deviceNo not exist", "");
			return re;
		}
		device.setDeviceId(deviceId);
		updateForResultEntity(device);
		re.setCode(ResultEntity.SUCCESS);
		return re;
	}
	...
	public ResultEntity<String> updateAllDeviceStatus(String status) {
		// TODO Auto-generated method stub
		deviceMapper.updateAllDeviceStatus(status);
		return super.proccessResultEntity(ResultEntity.SUCCESS, "update all status", "");
	}

	@Override
	public Device getByDeviceNo(String deviceNo) {
		// TODO Auto-generated method stub
		return deviceMapper.getDeviceByDeviceNo(deviceNo);
	}
	
	@Override
	public List<DeviceJson> getDeviceJosn(Device device){
		return deviceMapper.getSimpleList(device);
	}
