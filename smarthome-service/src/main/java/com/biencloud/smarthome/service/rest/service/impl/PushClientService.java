...
public class PushClientService implements IPushClientService{
	...
	/** IOS推送证书路径*/
    private String certPath = This.class.getClassLoader().getResource(Constants.IOS_SERT_DIR+Constants.IOS_PUSH_CERT_NAME).getPath();
    /** IOS推送证书路径-1byone*/
    private String certPath_1byone = This.class.getClassLoader().getResource(Constants.IOS_SERT_DIR+Constants.IOS_PUSH_CERT_NAME_1BYONE).getPath();
    /** IOS楼宇对讲推送证书路径*/
    private String certPath_intercom = This.class.getClassLoader().getResource(Constants.IOS_SERT_DIR+Constants.IOS_PUSH_CERT_NAME_INTERCOM).getPath();
	...
	@Override
	public void sendIosPushIntercom(final String alert, final String sound, final List<String> tokens, final int badge) {
		Thread t = new Thread(  
                new Thread(){  
                    @Override  
                    public void run() {  
                    	sendIosPushByCert(alert,sound, tokens, badge, certPath_intercom);
                    }  
                }  
        );
		t.start();
	...
	@Override
	public void sendIosPushIntercom(final String alert, final String sound, final String deviceNo, final int badge) {
		final List<String> tokens = this.getTokenListByDeviceNo(deviceNo, MsgToken.TYPE_IOS_APNS);
		Thread t = new Thread(  
                new Thread(){  
                    @Override  
                    public void run() {  
	...
	public void doSendAndroidJpush(String alert, String title,
			...
			if(list!=null && list.size()>0){
				...
				for(MsgToken token : list){
					...
					if (token.getToken() != null && !token.getToken().equals(pushClient.getExcludeToken())
							&&(token.getUserId()!=null && !token.getUserId().equals(pushClient.getExcludeUserId()))) {
				...
				if(tokensAndroidJpush.size()>0){
					...
					this.sendAndroidJpushThread(sb.toString(),title,map,null,tokensAndroidJpush,jPushClient);
				}
				if(tokensAndroidJpush1byone.size()>0){
					...
					this.sendAndroidJpushThread(sb.toString(),title,map,null,tokensAndroidJpush1byone,jPushClient);
				}
...
//			}).start();
	...
	public void sendAndroidJpush(final String alert, final String title, String sound,
		...
		if(registrationIds==null || registrationIds.size()==0){
			System.out.println("registrationIds is empty");
			return;
		}
		...
		Thread t = new Thread(  
                new Thread(){  
                    @Override  
                    public void run() {  
                    	doSendAndroidJpush(alert, title, extras, registrationIds,jPushClient);
                    }  
                }  
        );
		t.start();
	...
	public void sendAndroidJpushThread(final String alert, final String title, final Map<String,String> extras, String sound,
			final List<String> registrationIds, final JPushClient jPushClient) {
		if(sound!=null){
			extras.put("sound", sound);
		}else{
	...
	@Override
	public List<String> getTokenListByDeviceNo(String deviceNo,String tokenType) {
		List<String> list = new ArrayList<String>();
		ResultEntity<Long> result = devicesService.getIdByDeviceNo(deviceNo);
		if(result!=null && result.getCode()==ResultEntity.SUCCESS){
			Long deviceId = result.getData();
			List<MsgToken> msgToken = msgTokenService.getListByDeviceId(deviceId);
			for(MsgToken token:msgToken){
	...
	@Override
	public void sendAndroidJpushIntercom(String deviceNo,String alert, String title,
			String sound, JPushClient jPushClient) {
		List<String> tokens = getTokenListByDeviceNo(deviceNo,MsgToken.TYPE_ANDROID_JPUSH);
		sendAndroidJpush(alert, title, null, tokens, jPushClient);
	}

	
	public List<MsgToken> getTokenListByUsername(String username) {
		List<MsgToken> list = new ArrayList<MsgToken>();
			MsgToken msgToken = new MsgToken();
			User user = new User();
			user.setUsername(username);
			ResultEntity<User> result = userService.queryByUsername(username);
			if(result!=null && result.getCode()==ResultEntity.SUCCESS){
				User u = result.getData();
				msgToken.setUserId(new Long(u.getId()));
				list = msgTokenService.getListByEntity(msgToken);
			}
		return list;
	}
	
	@Override
	public void pushIntercom(String alert,String title,String username){
		List<MsgToken> list = getTokenListByUsername(username);
		...
		for(MsgToken token:list){
			if(MsgToken.TYPE_ANDROID_JPUSH.equals(token.getType())){
				androidToken.add(token.getToken());
			}
			if(MsgToken.TYPE_IOS_APNS.equals(token.getType())){
				iosToken.add(token.getToken());
			}
		}
		if(androidToken.size()>0){
			...
			this.sendAndroidJpush(alert, title, null, androidToken, jPushClient);
		}
		if(iosToken.size()>0){
			this.sendIosPushIntercom(alert, Constants.PUSH_SOUND_INTERCOM, iosToken, 1);
		}
	}
}
