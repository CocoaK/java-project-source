...
import com.biencloud.smarthome.service.common.interceptor.AuthorizationException;
...
public class DefaultRestErrorResolver implements RestErrorResolver, MessageSourceAware, InitializingBean {
    ...
    protected final Map<String,String> createDefaultExceptionMappingDefinitions() {
        ...
        // 401
        applyDef(m, AuthorizationException.class, HttpStatus.UNAUTHORIZED);
    ...
    public RestError resolveError(ServletWebRequest request, Object handler, Exception ex) {
        ...
        restError.setCode(getStatusValue(template, request, ex));

        String msg = getMessage(template, request, ex);
        if (msg != null) {
            restError.setMessage(msg);
        }
        msg = getData(template, request, ex);
        if (msg != null) {
            restError.setData(msg);
        }
    ...
    protected String getData(RestError template, ServletWebRequest request, Exception ex) {
        String devMsg = template.getData();
