...
public class LoginServiceImpl extends BaseService<Login, String> implements
	...
	public String getUserNameByLoginName(String loginName) {
		...
		if(UserType.SA.getValue().equals(login.getUserType()) || UserType.CA.getValue().equals(login.getUserType()))
	...
	private void setProperties(Login login){
		...
		} else if (UserType.CA.getValue().equals(login.getUserType())){
			SaUser user = getSaUserService().get(login.getUserId(),UserType.CA.getValue());
			if (user != null){
				login.setUserName(user.getUserName());
				login.setRoleCode(user.getRoleCode());
			}
