...
<script type="text/javascript">
 ...
 function repush(id)
    {      
      var url="push/pushAction_repush.action?pushFinishVO.id="+id;
      locationAction(url);	  
	  
    }
    function detail(id)
    {      
      var url="push/pushFinishAction_viewDetail.action?pushFinishVO.id="+id;
      locationAction(url);	  
		...
		<div class="all_aside">
			<div class="all_menu">
				<a href="push/pushAction_list.action" ><s:text name="datapush.unpush"/></a> <a
					href="push/pushFinishAction_list.action" class="all_menu_a"><s:text name="datapush.pushed"/></a>
				
			</div>
		</div>
		<div class="main_outside">
		<s:form action="push/pushFinishAction_list.action"  method="post"  theme="simple">
			<div class="all_inside">
				<div class="all_tab_top">
					...
					<select name="pushFinishVO.pushKind">
					    ...
					    <!-- <option value="password" <s:if test="pushFinishVO.pushKind==\"password\"">selected</s:if>><s:text name="datapush.kind.password"/></option>-->
					</select>  
					</div>
					...
					<s:textfield cssClass="all_tab_top_input" name="pushFinishVO.pushClientId" maxlength="20"/> <input type="button"
						value='<s:text name="datapush.management.button.search"/>' class="search_but"  onclick="doSubmit();"/>
				</div>
					...
					<tbody>
					  <s:iterator value="%{page.results}" var="push">
						<tr>
							...
							<td>
							<a  href="javascript:detail('<s:property value="#push.id"/>');" class="all_hover_but"><s:text name="datapush.buttion.detail"/></a>
							<a  href="javascript:repush('<s:property value="#push.id"/>');" class="all_hover_but"><s:text name="datapush.button.repush"/></a>
							</td>
						</tr>
