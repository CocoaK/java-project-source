...
public abstract class BaseRestService<T> implements IBaseRestService<T>{
	...
	@Override
	public ResultEntity<PagingVO<T>> getResultListForPage(Map map) throws Exception {
		return postForResultObject(getRestServiceUrl()+"/"+queryUrl, map, new ParameterizedTypeReference<ResultEntity<PagingVO<T>>>() {});
	}
	
	@Override
	public ResultEntity<PagingVO<T>> getResultListForPage(T record) throws Exception {
		return postForResultObject(getRestServiceUrl()+"/"+queryUrl, BeanUtils.describe(record), new ParameterizedTypeReference<ResultEntity<PagingVO<T>>>() {});
	}
	
	@Override
	public ResultEntity<PagingVO<T>> getResultListForPage(PagingVO<T> p,T record) throws Exception {
		Map<String,Object> map = new HashMap<String,Object>();
		map.put(Constants.REST_PARAM_PAGE, p);
		map.put(Constants.REST_PARAM_VO, record);
		return postForResultObject(getRestServiceUrl()+"/"+queryUrl, map, new ParameterizedTypeReference<ResultEntity<PagingVO<T>>>() {});
	}
