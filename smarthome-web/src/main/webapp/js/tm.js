...
{   
    var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:9999; background-color: #FFFFFF;border: 1px solid #CCCCCC;width:175px;font-size:12px;margin:152px 0 0 2px;"></div>';   
    if(document.all)   
    {   
...
function Calendar(beginYear, endYear, lang, dateFormatStyle) {
  ...
  this.colors = {
		...
		"cur_bg"        : "#FF9B59",  //当日日期单元格背影色
		"sel_bg"        : "#F60",  //已被选择的日期单元格背影色 2006-12-03 www.ttkc.net添加  
		"sun_word"      : "#FF0000",  //星期天文字颜色  
		"sat_word"      : "#0000FF",  //星期六文字颜色  
		"td_word_light" : "#FFF",  //单元格文字颜色  
		"td_word_dark"  : "#FFF",  //单元格文字暗色  
		"td_bg_out"     : "#EFEFEF",  //单元格背影色
		"td_bg_over"    : "#F60",  //单元格背影色
...
Calendar.prototype.draw = function() {
  ...
  mvAry[mvAry.length]  = '    <table width="100%" cellspacing="0" class="r_sel">';
  mvAry[mvAry.length]  = '      <tr>';
  mvAry[mvAry.length]  = '        <th><input class="r_left" style="';
  ...
  mvAry[mvAry.length]  = '        <th><select name="calendarYear" id="calendarYear" style="padding:2px;"></select><select name="calendarMonth" id="calendarMonth" style="padding:2px';
  if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏"月"
  mvAry[mvAry.length]  = '"></select></th>';
  mvAry[mvAry.length]  = '        <th><input class="r_right" style="';
  if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年时隐藏"月"
  mvAry[mvAry.length]  ='" name="nextMonth" type="button" id="nextMonth" value="" /></th>';
  mvAry[mvAry.length]  = '      </tr>';
  mvAry[mvAry.length]  = '    </table>';
  mvAry[mvAry.length]  = '    <table id="calendarTable" width="100%" cellspacing="0" class="r_show" style="';
  if(calendar.DateMode >= pickMode["month"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到年、月时隐藏"天"
  mvAry[mvAry.length]  = '" >';
  mvAry[mvAry.length]  = '      <tr class="r_top">';
  for(var i = 0; i < 7; i++) {
    mvAry[mvAry.length]  = '      <th>' + Calendar.language["weeks"][this.lang][i] + '</th>';
  }
  mvAry[mvAry.length]  = '      </tr>';
  for(var i = 0; i < 6;i++){
    mvAry[mvAry.length]  = '    <tr class="r_body">';
    for(var j = 0; j < 7; j++) {
      if (j == 0){
...
//2009-03-03 添加的代码，放置时间的行
  mvAry[mvAry.length]  = '      <tr style="';
    ...
    mvAry[mvAry.length]  = '"><td colspan="7">';
  mvAry[mvAry.length]  = '      <select name="calendarHour" id="calendarHour" style="padding:2px;"></select>' + Calendar.language["hour"][this.lang];
  mvAry[mvAry.length]  = '<span style="'
    if(calendar.DateMode >= pickMode["hour"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到小时时隐藏"分"
  mvAry[mvAry.length]  = '"><select name="calendarMinute" id="calendarMinute" style="padding:2px;"></select>' + Calendar.language["minute"][this.lang]+'</span>';
    mvAry[mvAry.length]  = '<span style="'
    if(calendar.DateMode >= pickMode["minute"]){mvAry[mvAry.length]  = 'display:none;';}//pickMode 精确到小时、分时隐藏"秒"
   mvAry[mvAry.length]  = '"><select name="calendarSecond" id="calendarSecond" style="padding:2px;"></select>'+ Calendar.language["second"][this.lang]+'</span>';
  mvAry[mvAry.length]  = '      </td></tr>';
  
  mvAry[mvAry.length]  = '    </table>';
  //mvAry[mvAry.length]  = '  </from>';
  mvAry[mvAry.length]  = '      <div class="r_bottom">';
...
function InitContainerPanel() //初始化容器
{
 var str = '<div id="calendarPanel" style="position:absolute; display:none; z-index:9999; background-color: #FFF; margin:152px 0 0 2px;"></div>';
 if(document.all)
 {
 // str += '<iframe style="position:absolute;z-index:2000;width:expression(this.previousSibling.offsetWidth);';
 // str += 'height:expression(this.previousSibling.offsetHeight);';
